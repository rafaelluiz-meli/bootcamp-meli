-- Exibe o título e o nome do gênero de todas as séries (primeiro modo)
SELECT s.title, g.name FROM series s, genres g WHERE s.genre_id = g.id;
-- Exibe o título e o nome do gênero de todas as séries. (segundo modo)
SELECT s.title, g.name FROM series s INNER JOIN genres g ON s.genre_id = g.id;
-- Exibe o título dos episódios, o nome e sobrenome dos atores que trabalham em cada um deles.
...
-- Exibe o título de todas as séries e o número total de temporadas que cada uma delas possui.
SELECT s.title, COUNT(se.id) AS Seasons FROM series s INNER JOIN seasons se ON s.id = se.serie_id GROUP BY title;
-- Exibe o nome de todos os gêneros e o número total de filmes de cada um, desde que seja maior ou igual a 3.
...
-- Exibe apenas o nome e sobrenome dos atores que atuam em todos os filmes de Star Wars e que estes não se repitam.
